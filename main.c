...
GLOBAL(void) write_JPEG_file (char * filename, int quality, int k, rgb_list **cluster_ary, rgb_list *rgbList);
...
main(int argc, char *argv[])
{
    ...
    image_buffer = (unsigned char *) calloc (image_width * 3, sizeof (unsigned char));
    argv[1][0] = argv[2][0];
...
set_image_buffer (int k, rgb_list **cluster_ary, rgb_list *rgbList)
{
    ...
    image_buffer = (unsigned char *) calloc (image_pixel_len * 3, sizeof (unsigned char));
    // scan every pixel
    for (pixel_pos = image_pixel_len - 1, cur_rgbList = rgbList; cur_rgbList != NULL; pixel_pos--, cur_rgbList = cur_rgbList->next)
    {
...
GLOBAL(void)
write_JPEG_file (char * filename, int quality, int k, rgb_list **cluster_ary, rgb_list *rgbList)
{
